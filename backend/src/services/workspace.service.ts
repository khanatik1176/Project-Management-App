...
export const deleteWorkSpaceService = async (workspaceId: string, userId: string) => {
    ...
    try {
        ...
        if(user?.currentWorkplace?.equals(workspaceId))
        {
            const memberWorkspace = await MemberModel.findOne({ userId }).session(session);

            user.currentWorkplace =  memberWorkspace ? memberWorkspace.workspaceId : null;  
        ...
        return { currentWorkspace: user?.currentWorkplace };
    } catch (error) {
